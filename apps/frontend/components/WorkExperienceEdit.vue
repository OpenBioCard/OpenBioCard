<template>
  ...
  <div style="padding: 2rem; border-top: 1px solid var(--color-border-tertiary);">
    <div style="display: flex; align-items: center; gap: 0.75rem; margin-bottom: 1.5rem;">
      ...
      <h3 style="font-size: 1.25rem; font-weight: bold; color: var(--color-text-primary);">{{ $t('profile.editWorkExperience') }}</h3>
    </div>
    ...
    <div v-if="workExperiences && workExperiences.length > 0" style="display: flex; flex-direction: column; gap: 1.5rem;">
      <div v-for="(exp, index) in workExperiences" :key="index" style="padding: 1.5rem; background: var(--color-bg-secondary); border-radius: 0.75rem; border: 1px solid var(--color-border-tertiary);">
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1rem;">
          <div style="display: flex; flex-direction: column; gap: 0.5rem;">
            <label style="font-size: 0.75rem; font-weight: 500; color: var(--color-text-secondary);">{{ $t('profile.position') }}</label>
            <input
              :value="exp.position"
          ...
          <div style="display: flex; flex-direction: column; gap: 0.5rem;">
            <label style="font-size: 0.75rem; font-weight: 500; color: var(--color-text-secondary);">{{ $t('profile.company') }}</label>
            <input
              :value="exp.company"
        ...
        <div style="display: flex; gap: 1rem; align-items: flex-start;">
          ...
          <div style="flex-shrink: 0;">
            <label style="font-size: 0.75rem; font-weight: 500; color: var(--color-text-secondary); display: block; margin-bottom: 0.5rem;">{{ $t('profile.companyLogo') }} <span style="font-weight: normal; color: var(--color-text-tertiary);">({{ $t('common.optional') }})</span></label>
